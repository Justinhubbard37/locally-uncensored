...
describe('markdown export', () => {
  it('renders a ranked table with runs and the latest pass', () => {
    ...
    expect(md).toContain('| # | Model | Score | Average t/s | Latest t/s | Avg tokens | Think | Correct | Runs |')
    // Ranked fastest first, same order as the leaderboard on screen.
    expect(md.indexOf('fast:3b')).toBeLessThan(md.indexOf('slow:70b'))
    // Score first, then the speeds: the table prints what it is sorted by.
    expect(md).toMatch(/\| 1 \| fast:3b \| \d+ \| 41 \| 41 \|/)
    expect(md).toMatch(/\| 2 \| slow:70b \| \d+ \| 4 \| 4 \|/)
    // These runs predate the economy fields, so those columns read as a dash
    // rather than a wrong number.
    expect(md).toMatch(/\| 1 \| fast:3b \| \d+ \| 41 \| 41 \| - \| - \| - \| 2 \|/)
