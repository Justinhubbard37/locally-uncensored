...
(function(){
  ...
  var AGENT_TOOLS = [
    ...
    {name:'shell_execute', description:'THE terminal. Run a shell command: PowerShell on Windows, bash on Unix. Returns stdout, stderr, exit code. Everything runs here: git, tests, package managers, gh, python, platform utilities. This is also how you open things on the desktop: files, folders, apps (the system prompt states the OS and the exact open command). Useful forms: `git status --porcelain=2 --branch` and a recognised test run (`npm test`, `npx vitest run`, `cargo test`, `pytest`) return a parsed summary; `git log --oneline -n 20`; `git diff`; `git add -A && git commit -m "msg"`; `git push`; `gh pr create --title "t" --body "b"`. Feed a script via `stdin` instead of quoting it: set command to `python3 -` or `bash -s` and put the source in `stdin` (fresh process each call, no REPL state). Long-running work: set `background: true` to get a task id back immediately; follow it by calling again with `task: "status"` and `task_id` (or `task: "list"` / `task: "kill"`). PREFER dedicated tools where available: file_read over `cat`, file_list over `ls`/`dir`, file_search over `grep`. `git commit --no-verify` is refused. NEVER permanently delete without confirmation (rm -rf, Remove-Item -Recurse, git reset --hard). Default timeout 120 s (recognised test runs get 300 s); set higher only for known long builds.',
