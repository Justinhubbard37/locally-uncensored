...
/// *crawls* — which reads as a broken app, not as a throttle (a pull measured
/// at 2.6 KB/s was misdiagnosed as a dead network line). Gated repos are out
...
/// Store the token from Settings. An empty string clears it — that is how the
/// user removes the token again, so it must not be stored as `Some("")`.
